...
export interface RollupTotais { mensal: number; essMensal: number; condMensal: number }
...
// RN-OR-06: tudo derivado de valorOrcadoMensal (valor_orcado é não-somável).
export function rollupGrupo(
  grupoCodigo: string,
  grupos: GrupoOrcamento[],
  linhas: LinhaOrcamento[],
): RollupTotais {
  const r: RollupTotais = { mensal: 0, essMensal: 0, condMensal: 0 }
  for (const l of linhas) {
    if (l.grupoCodigo !== grupoCodigo) continue
    r.mensal += l.valorOrcadoMensal
    if (ehEssencial(l.classificacao)) r.essMensal += l.valorOrcadoMensal
    else r.condMensal += l.valorOrcadoMensal
  }
  for (const filho of grupos) {
    if (filho.codigoPai !== grupoCodigo) continue
    const s = rollupGrupo(filho.codigo, grupos, linhas)
    r.mensal += s.mensal; r.essMensal += s.essMensal; r.condMensal += s.condMensal
  }
  return r
}

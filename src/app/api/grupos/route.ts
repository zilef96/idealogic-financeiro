...
import { criarGrupo } from "@/lib/repositories/orcamento-repository"

const novoGrupo = z.object({
  ano: anoSchema,
  codigo: z.coerce.number().int().positive(),
  codigoPai: z.string().nullable().default(null),
  tipo: z.enum(["R", "C", "D", "E"]),
...
export async function POST(req: Request) {
  ...
  try { return NextResponse.json({ id: await criarGrupo(parsed.data) }, { status: 201 }) }
  catch (e) { return handleApiError(e, "Falha ao criar grupo.") }
}
